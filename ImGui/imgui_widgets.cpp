...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    value->second = ImLerp(value->second, percent * frame_bb.GetWidth(), 0.125f);  // Smoother interpolation

    // Draw slider background with rounded corners
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, hovered || IsItemActive() ? ImColor(36, 35, 42) : ImColor(31, 30, 37), 8.0f);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(1.f, 1.f, 1.f, 0.07f * style.Alpha), 8.0f);

    // Draw slider fill with accent color and smooth animation
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + value->second, frame_bb.Max.y), custom.get_accent_color(style.Alpha), 8.0f);

    // Handle color (White) with hover and active effects
    ImColor handle_color = ImColor(255, 255, 255, 255);  // White color for the handle
    ImVec2 handle_pos(frame_bb.Min.x + value->second, frame_bb.Min.y + frame_bb.GetHeight() / 2);

    // Draw the handle as a white circle
    window->DrawList->AddCircleFilled(handle_pos, 8.0f, handle_color);

    // Draw value display above the slider
    if (!IsItemActive())
        RenderTextClipped(total_bb.Min, total_bb.Max, value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
    else {
        // Display the current value above the slider with a shadow effect
        float shadow_offset = 2.0f;
        GetForegroundDrawList()->AddRectFilled(
            ImVec2(frame_bb.Min.x + value->second - CalcTextSize(value_buf, 0, 1).x / 2 - 5, frame_bb.Min.y - 25),
            ImVec2(frame_bb.Min.x + value->second + CalcTextSize(value_buf, 0, 1).x / 2 + 4, frame_bb.Min.y - 5),
            GetColorU32(ImGuiCol_WindowBg), 4.0f
        );
        GetForegroundDrawList()->AddText(
            ImVec2(frame_bb.Min.x + value->second - CalcTextSize(value_buf, 0, 1).x / 2 + shadow_offset, frame_bb.Min.y - 23 + shadow_offset),
            ImColor(0, 0, 0, 180), value_buf, value_buf_end  // Shadow color
        );
        GetForegroundDrawList()->AddText(
            ImVec2(frame_bb.Min.x + value->second - CalcTextSize(value_buf, 0, 1).x / 2, frame_bb.Min.y - 23),
            GetColorU32(ImGuiCol_Text), value_buf, value_buf_end // Main text color
        );
    }
